...
import { Lock, User as UserIcon, MapPin, Phone, Calendar, Mail, ChevronDown, CheckSquare, Square, ShieldCheck, BookOpen, ArrowRight, ArrowLeft } from 'lucide-react';
...
const InputField = ({ label, type = "text", value, onChange, icon: Icon, required = false, placeholder = "" }: any) => (
  <div className="mb-4">
...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const renderLanding = () => (
    <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 px-4 relative z-10">
      {/* Student Option */}
      <div 
  ...
  const renderAdminFlow = () => (
    <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden my-8 relative z-10">
      <button 
        onClick={() => { setError(''); setCurrentView('LANDING'); setAdminEmail(''); setAdminPass(''); }} 
  ...
  const renderStudentAuth = () => (
    <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden my-8 relative z-10">
      ...
      <div className="bg-indigo-600 px-8 py-6 text-white text-center">
        <h2 className="text-3xl font-bold tracking-tight">Catequesis Digital</h2>
