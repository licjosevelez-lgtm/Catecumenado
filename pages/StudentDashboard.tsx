...
export const StudentDashboard: React.FC<Props> = ({ user, view = 'dashboard', onUserUpdate }) => {
  ...
  // BLINDAJE DE DATOS: Arrays seguros
  const safeCompletedModules = user.completedModules || [];
  const safeSacramentTypes = user.sacramentTypes || [];

  useEffect(() => {
    const loadData = async () => {
      const m = await MockService.getModules();
      setModules(m.sort((a: Module, b: Module) => a.order - b.order));
      const conf = MockService.getAppConfig();
      if (conf) setConfig(conf);
    };
    loadData();
  }, []);

  // Initialize profile data with Safe Access
  useEffect(() => {
    setProfileData({
        name: user.name || '',
        age: user.age || 18,
  ...
  const renderDashboard = () => {
    ...
    return (
        ...
        <div 
            className="h-64 rounded-2xl bg-cover bg-center shadow-md relative flex items-center justify-between overflow-hidden"
        ...
        >
            ...
            <div className="relative z-10 p-8 text-white flex-1">
                ...
                <p className="opacity-90 mt-2 text-lg">
                    {safeCompletedModules.length === modules.length 
                    ? "¡Has completado toda tu formación teórica!" 
                    : "Continúa tu camino de fe."}
                </p>
            </div>

            {/* FIXED SIZE PIE CHART (No ResponsiveContainer) */}
            <div className="relative z-10 p-8 hidden md:flex flex-col items-center">
                <div style={{ width: 120, height: 120 }}>
                    ...
                    {/* Centered Text */}
                    <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center pointer-events-none" style={{ paddingTop: '20px' }}>
                         <span className="text-xl font-bold text-white">
                            {Math.round((safeCompletedModules.length / Math.max(modules.length, 1)) * 100)}%
                         </span>
                    </div>
                </div>
